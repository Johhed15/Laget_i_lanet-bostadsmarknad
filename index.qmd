...
Här ska antingen en inledning eller kort sammanfattning in!
...
```{r, echo = FALSE}
#colnames(bedom_av_bostadsmarknad)[2] <- 'Bostadsmarknadslägesbedömning'

#knitr::kable(bedom_av_bostadsmarknad, caption = 'Bedömning av bostadsmarknadsläge [Kommunen som helhet]')
```
...
bedom_av_bostadsmarknad %>%
  ...
  data_color(
    ...
    colors = scales::col_factor(
      palette = c( "#4AA271","#D0342C"), # choose colors you like
...
tot_ledigt %>%
  ...
  data_color(
    ...
    colors = col_numeric(
      ...
      domain = NULL                 # scales automatically from min to max
    )
  ) %>%  tab_options(table.font.names = "sourcesanspro")

```

Alla kommuner har en låg andel lägenheter som står lediga, lägst har Knivsta och Uppsala på 0,0% och högst har Tierp på 0,2%.
...
# Compute row-wise difference in a temporary variable
diff_values <- flerbostadshus_wide$Hyresrätt - flerbostadshus_wide$Bostadsrätt

# Create gt table using only the original columns
...
df_kvot %>%
  gt() %>%
  tab_header(
    title = "Hyresgästföreningens kvot per kommun i Uppsala län, basår 2006"
  ...
  data_color(
    ...
    colors = col_numeric(
      ...
      domain = NULL                 # automatically scales from min to max
    )
  ) %>% tab_options(table.font.names = "sourcesanspro")
```
...
Prognosen beräknas likt @eq-bostadsbehov, men att istället för att använda de faktiskt antal människor över 20 per kommun, så används prognosen för antalet invånare över 20 för de kommande 20 åren.
...
# Highlight the highest value per column
for (col in colnames(table_df2)[-1]) {  # skip 'Typ' column
  # Find the row(s) with the max value in this column
  max_rows <- table_df2$Typ[table_df2[[col]] == max(table_df2[[col]])]
  
  # Apply coloring for each max row
  for (row in max_rows) {
    gt_table <- gt_table %>%
      tab_style(
...
```{r}
# Befolkningsstatistik antal personer efter region, förändringar, period, kön och år 2025
url <- 'https://api.scb.se/OV0104/v1/doris/sv/ssd/START/BE/BE0101/BE0101G/BefforandrKvRLKCKM'


pxweb_query_list <-
  list('Region' = kommunkod,
       'Forandringar'= c('115', '130', '140','150','160','170','175', '180','190'),
       'Period' = c("halv1","halv2"),
       'Kon' = 'TotSa',
       'Tid' = '*',
       'ContentsCode' = '*'
  )
...
df_befolk_halvar <- as.data.frame(px_data, column.name.type = "text", variable.value.type = "text")

df_befolk_halvar <- df_befolk_halvar %>% drop_na()
...
```{r}
# Befolkningsstatistik, antal personer efter region, förändringar, period, kön och år
